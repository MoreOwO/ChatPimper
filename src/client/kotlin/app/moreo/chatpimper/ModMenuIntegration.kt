...
class ModMenuIntegration: ModMenuApi {
    ...
    override fun getModConfigScreenFactory(): ConfigScreenFactory<*> {
        ...
        builder.title(Text.translatable("config.title")).apply{
            category {
                ...
                group {
                    ...
                    option<Boolean> {
                        name(Text.translatable("config.param.messages.split"))
                        description(OptionDescription.of(Text.translatable("config.param.messages.split.desc"), Text.translatable("config.param.messages.split.desc.warning").setStyle(
                            Style.EMPTY.withColor(0xffff00).withBold(true))))

                        binding {
                            def = config.defaults.splitMessages
                            getter = Supplier { config.config.splitMessages }
                            setter = Consumer { config.config.splitMessages = it }
                        }
            ...
            category {
                name(Text.translatable("config.category.bounded.commands"))

                option(ListOption.createBuilder<KeyBoundCommand>().apply {
                    name(Text.translatable("config.param.bounded.commands.list"))

                    binding {
                        def = config.defaults.boundedCommands
                        getter = Supplier { config.config.boundedCommands }
                        setter = Consumer { config.config.boundedCommands = it }
                    }

                    controller(KeyBoundCommandController.KeyBoundCommandControllerBuilder::create)

                    initial(KeyBoundCommand(InputUtil.UNKNOWN_KEY.code, InputUtil.GLFW_KEY_LEFT_CONTROL, "say delete me and add your own commands without the first /"))
                }.build())
            }
        }
